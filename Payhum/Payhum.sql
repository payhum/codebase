...
DROP TABLE IF EXISTS `emp_payroll_view`;
DROP VIEW IF EXISTS `emp_payroll_view`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `payhumrepo`.`emp_payroll_view` AS select `payhumrepo`.`employee`.`employeeId` AS `EMPLOYEEID`,concat(`payhumrepo`.`employee`.`firstname`,_utf8' ',`payhumrepo`.`employee`.`middlename`,_utf8' ',`payhumrepo`.`employee`.`lastname`) AS `FULL_NAME`,`payhumrepo`.`position`.`salary` AS `GROSS_SALARY`,_utf8'Benefits' AS `BENEFIT_TYPE`,sum(`emp_benefit_view`.`BENEFIT_AMNT`) AS `BENEFIT_AMNT` from (`payhumrepo`.`position` join (`payhumrepo`.`employee` left join `payhumrepo`.`emp_benefit_view` on((`payhumrepo`.`employee`.`employeeId` = `emp_benefit_view`.`EMP_ID`)))) where (`payhumrepo`.`position`.`id` = `payhumrepo`.`employee`.`positionId`) group by `payhumrepo`.`employee`.`employeeId`;
